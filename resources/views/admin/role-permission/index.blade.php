...
@section('content')
    ...
    <div class="border rounded-3 overflow-hidden">
        <div class="bg-light px-3 py-2.5 h4 text-capitalize m-0">
            {{ ucfirst($type) }} Permissions
        </div>
        <div class="p-3 d-flex flex-column gap-3 bg-white">
            @if(is_array($allPermissions) && count($allPermissions) > 0)
                        ...
                        <div class="fz-18 d-flex align-items-center flex-wrap gap-3 p-3">
                            @if(is_array($permissionValues))
                                @foreach ($permissionValues as $permission)
                                    <div class="d-flex align-items-center gap-2">
                                        <input type="checkbox"
                                            id="{{ $type . '-' . str_replace(' ', '-', $permission) }}"
                                            name="permissions[]"
                                            value="{{ $permission }}"
                                        ...
                                        <label for="{{ $type . '-' . str_replace(' ', '-', $permission) }}" class="m-0">
