...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('admin.envoy-notes.store') }}" method="POST">
                @csrf
                <div class="row g-3">
                    ...
                    <div class="col-md-12">
                        ...
                        <textarea name="content" id="content" rows="6" class="form-control @error('content') is-invalid @enderror" required placeholder="{{ __('Enter note content here...') }}">{{ old('content') }}</textarea>
                        @error('content') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>

                    <div class="col-md-12 text-end">
                        <button type="submit" class="btn btn-primary px-5">{{ __('Save Note') }}</button>
                    </div>
                </div>
            </form>
        </div>
