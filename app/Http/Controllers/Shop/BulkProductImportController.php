...
class BulkProductImportController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $folders = $request->folder;
            ...
            if (! $folders) {
            return back()->with('error', __('Please select folder'));
        }
    
       Log::info('Folder' . json_encode($folders));

        // Validate if Import with gallery folder selected
        if ($folders !== null && count($folders) === 0) {
            return back()->with('error', __('Please select at least one gallery folder when importing with thumbnails.'));
        }
        ...
        // If zero products and gallery folders selected, possible no matching thumbnails
        if ($total === 0 && $folders !== null) {
            return back()->with('error', __('No products imported. Check that thumbnail images exist in the selected gallery folders.'));
        }
