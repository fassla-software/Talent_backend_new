...
class ProductController extends Controller
{
    ...
    public function bulkDestroy(Request $request)
    {
        ...
        if (empty($ids)) {
            return response()->json(['message' => __('No products selected for deletion.')], 400);
        }

        $shop = generaleSetting('shop');

        // Ensure products belong to the current shop
        $products = $shop->products()->whereIn('id', $ids)->get();

        if ($products->isEmpty()) {
            return response()->json(['message' => __('No valid products found for deletion.')], 404);
        }
        ...
        foreach ($products as $product) {
            $product->delete();
                ...
                return back()->withSuccess(__('Products deleted successfully'));
    ...
    public function destroy(Product $product)
    {
        ...
        // Ensure product belongs to the current shop
        if (!$shop->products()->where('id', $product->id)->exists()) {
            return back()->withError(__('Product not found or does not belong to your shop.'));
        }
